...
@testitem "create_in_local_frame" begin
    ...
    c0 = create_circle_in_local_frame(points2[:, 1], points2[:, 2], points2[:, 3], 20, 1)
    distances = norm.(map(p->p.-points2[:,1], eachcol(c0)))
    all
    @test allapproxequal(distances)

    c1 = create_circle_in_local_frame(points[:, 1], points[:, 2], points[:, 3], 20, 1)
    distances = norm.(map(p->p.-points[:,1], eachcol(c1)))
    @test allapproxequal(distances)

    # circles and ellipses can be equivalent when applying the right scales
    e1 = create_ellipse_in_local_frame(points[:, 1], points[:, 2], points[:, 3], 20, 1, 1)
    @test c1==e1

    c2 = create_circle_in_local_frame(points[:, 1], points[:, 2], 2*points[:, 3], 20, 1)
    e2 = create_ellipse_in_local_frame(points[:, 1], points[:, 2], points[:, 3], 20, 1, 2)
    @test c2==e2

    # rectangle
    r1 = create_rectangle_in_local_frame(points[:, 1], points[:, 2], 2*points[:, 3], 20, 1, 2)
    ...
    c3 = create_circle_in_local_frame(points[:, 1], points[:, 2], points[:, 3], 20, 0)
    e3 = create_ellipse_in_local_frame(points[:, 1], points[:, 2], points[:, 3], 20, 0, 0)
    r3 = create_rectangle_in_local_frame(points[:, 1], points[:, 2], points[:, 3], 20, 0, 0)
    for obj in [c3, e3, r3]
...
@testitem "merge_multiple_meshes" begin
    ...
    0 1 0
    0 0 0], reshape([1, 2, 3], (3, 1)), [(0, 0, 0), (255, 0, 0), (0, 255, 0)])
    ...
    empty = PlainMesh(Matrix{Int}(undef, 3, 0), Matrix{Int}(undef, 3, 0), Vector{NTuple{3, Int}}())
...
@testitem "connect_circles_to_tube" begin
    ...
    0 0 0]
    colors = [(0, 0, 0), (255, 0, 0), (0, 255, 0)]
    a = PlainNonStdMesh(points, [[1,2,3]], colors)

    a_std = PlainMesh(points, reshape([1,2,3], (3, 1)), colors)
    ...
    res_endpoints = connect_circles_to_tube([a, a], ([0, 0, 0], [1,1,1]))
